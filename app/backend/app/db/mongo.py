...
async def create_indexes() -> None:
    ...
    # ── duplicate_groups ───────────────────────────────────────────────────────
    await db["duplicate_groups"].create_index("status", background=True)
    await db["duplicate_groups"].create_index("detection_method", background=True)
    await db["duplicate_groups"].create_index("track_ids", background=True)
    await db["duplicate_groups"].create_index(
        [("created_at", DESCENDING)], background=True
    )
