...
from worker.utils.metadata import extract_audio_metadata, extract_artwork
from worker.utils.quality import score_track
...
def transcode_track(self: Task, track_id: str, r2_key_raw: str) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="tamasha_tc_") as tmp:
        ...
        # ── 2b. Artwork extraction ────────────────────────────────────────────
        artwork_r2_key: str | None = None
        art_bytes, art_mime, art_ext = extract_artwork(raw_path)
        if art_bytes:
            artwork_r2_key = f"music/artwork/{track_id}{art_ext}"
            try:
                upload_bytes(art_bytes, artwork_r2_key, content_type=art_mime)
                _update_track(track_id, {"artwork_r2_key": artwork_r2_key})
                log.info("artwork_uploaded", r2_key=artwork_r2_key, mime=art_mime, size=len(art_bytes))
            except Exception as art_exc:
                artwork_r2_key = None
                log.warning("artwork_upload_failed", error=str(art_exc))
        ...
        # ── 7. Precompute quality score ───────────────────────────────────────
        # Fetch the fully updated doc so scoring uses final metadata + artwork
        updated_doc = db["tracks"].find_one({"_id": ObjectId(track_id)}) or {}
        qs, qb = score_track(updated_doc)
        quality_fields: dict = {"quality_score": qs, "quality_breakdown": qb}

        # Auto-tag poor quality tracks (score < 10) at index time.
        # Tag is additive — staff can clear it manually via the edit form.
        if qs < 10:
            existing_tags = updated_doc.get("workflow_tags") or []
            if "poor_quality" not in existing_tags:
                quality_fields["workflow_tags"] = existing_tags + ["poor_quality"]

        _update_track(track_id, quality_fields)
        log.info("transcode_complete", r2_keys=list(r2_keys_transcoded.keys()),
                 quality_score=qs, needs_review=track_update.get("needs_human_review", False))
