...
export function ListenerHome({ statusFilter = "ready" }: Props) {
  ...
  const { data, isLoading: tracksLoading } = useQuery({
    queryKey: ["tracks", { status: statusFilter ?? undefined, limit: 100 }],
    queryFn: () => tracksApi.list({ status: statusFilter ?? undefined, limit: 100 }),
  });
  ...
  const filtered = tracks.filter(
    (t) =>
      ...
      inferredArtist(t)?.toLowerCase().includes(search.toLowerCase())
  );
